...
`text-safe` 네 모드를 제공하며 기본값은 `conservative`다.

`text-safe`는 회색 워터마크가 작은 글자와 겹친 화면 캡처를 위한 선택형 모드다.
무채색 픽셀은 luminance 150을 경계로 흑백 이진화하고, 색이 있는 그림 픽셀은
그대로 둔다. 분할된 쪽 폭이 1,400px 미만이면 최근접 보간으로 2배 확대한다.
일반 OCR 원문 좌표도 추가 저장하므로 기본 모드보다 느리지만, 구조 Markdown에서
누락된 번호 문단과 Note 줄을 원래 위치에 복구할 수 있다.
